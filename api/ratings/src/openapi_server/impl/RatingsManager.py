...
class RatingsManager (BaseDefaultApi):
    ...
    async def rate_article(self, id: str, new_rating: NewRating):
        ...
        await self._update_article_and_wiki_average(id, article, rating)
    ...
    async def _update_article_and_wiki_average(self, id, article, rating):
        article_rating = await self.get_article_average_rating(id)
        value = rating.value if isinstance(rating, Rating) else rating.get("value")
        article['rating'] = (article_rating.total_sum + value ) / (article_rating.total + 1)
        await self.mongodb["article"].update_one({'_id': self._convert_id_into_ObjectId(id)}, {'$set': article})

        wiki_articles = await self.mongodb["article"].find({'wiki_id': article['wiki_id']}).to_list()
        total = 0;
        for articles in wiki_articles:
            total += articles['rating']
        await self.mongodb["wiki"].update_one({'_id': article['wiki_id']},
                                              {'$set': {'rating': total / len(wiki_articles)}})
