...
class RatingsManager (BaseDefaultApiV2):
    app: FastAPI = FastAPI()
    ...
    async def delete_rating(self, id: str, user_id: str, admin: bool):
        rating = await self._check_rating_exists(id)
        # If author is same as person who sent the request delete
        if admin is False:
            if rating['author']['_id'] != self._convert_id_into_ObjectId(user_id):
    ...
    async def rate_article(self, id: str, new_rating: NewRating, user_id: str, admin: bool):
        ...
        if admin is False:
            if new_rating.author_id != user_id:
                raise HTTPException(status_code=403, detail="No permissions to edit")
    ...
    async def edit_article_rating(self, id: str, new_rating: NewRating, user_id: str, admin: bool):
        self._check_new_rating_is_valid(new_rating)
        article = await self._check_article_exists(id)
    ...
    async def check_user(self, user_id: str) -> int:
            response = await requests.head(self.USERS_API_URL + "/v1/users/{user_id}")
            return response.status_code

    async def get_user(self, user_id: str) -> dict:

            response = await requests.get(self.USERS_API_URL + "/v1/users/{user_id}")
